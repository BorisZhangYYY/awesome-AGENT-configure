...
用户确认后，为每个选定任务生成新的场景 YAML：

```bash
~/.openclaw/workspace/awesome-AGENT-configure/cron/migrated-<job-name>.yaml
```
...
```bash
python3 /path/to/awesome-AGENT-configure/OpenClaw/scripts/build-cron.py ~/.openclaw/workspace/awesome-AGENT-configure/cron/migrated-<job-name>.yaml
```
